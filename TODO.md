...
- 真实请求响应关联仍偏基础，尚未形成完整的重试/退避/状态机闭环。
- 串口轮询、UDP 会话识别、S7/IEC104 采集链路仍需实际联调验证。
- 文档、模板、README 仍存在编码和一致性治理任务。
- 缺少自动化回归测试，当前验证仍以构建通过和局部运行链路为主。
...
- [ ] 明确 `ProtocolConfig` 与具体协议配置的最终关系：继承、组合或判别联合
- [ ] 逐步移除对 `ParseRules["CustomProtocolJson"]`、`ParseRules["ModbusConfig"]` 等隐式键的强依赖
- [ ] 为配置加载增加强校验和错误报告
...
- [ ] 支持按 `ProtocolType + ProtocolVersion + DeviceModel` 组合选择解析器
- [ ] 为解析器注册增加元数据：名称、版本、适用设备、支持能力、作者、状态
- [ ] 支持插件式协议装载，减少新增协议时对主工程的侵入
...
- [ ] TCP 通道补齐更完整的请求响应交互模型
- [ ] UDP 通道补齐更稳定的设备识别、会话管理、超时下线策略
- [ ] 串口链路补齐帧间隔、读写超时、轮询、共享串口、多设备从站管理
- [ ] 统一通道层事件：连接、断连、收包、发包、异常、统计

## P1

### 6. 建立协议回归测试闭环

- [ ] 以 `Vktun.IoT.Connector.DeviceMock` 为基础建立自动化回归测试工程
- [ ] 为 Modbus TCP、Modbus RTU、S7、IEC104 建立最小可运行回归用例
- [ ] 补齐边界测试：空帧、短帧、CRC 错误、长度错误、异常码、字节序错误
- [ ] 建立协议样例报文库，支持请求/响应回放
...
- [ ] 定义跨协议统一的点位描述模型
- [ ] 为 Modbus、S7、自定义协议建立统一地址表达
- [ ] 支持地址解析与规范化输出
- [ ] 支持位、字节、字、双字、浮点、字符串、BCD、枚举映射
- [ ] 支持同一设备多协议点位或多模板点位并存

### 8. 增强自定义协议能力

- [ ] 补齐校验规则：CRC16/CRC32/LRC/XOR/SUM 的统一配置化实现
- [ ] 支持变长帧、分隔符帧、带设备号帧、带序列号帧
- [ ] 支持位段解析、枚举映射、条件字段、动态长度字段
- [ ] 支持反向打包，用于指令下发
- [ ] 支持原始值、转换值、质量位、错误原因同时输出

### 9. 做出真正可用的协议调试工具

- [ ] 将 Demo 客户端的随机读写逻辑替换为真实协议收发
- [ ] 提供十六进制收发面板、原始报文查看、解析结果预览
- [ ] 支持从模板直接发起调试
- [ ] 支持导入样例报文并回放
- [ ] 支持错误定位：帧头不匹配、长度错误、校验失败、偏移越界
