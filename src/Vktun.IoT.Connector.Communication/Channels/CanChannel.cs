...
public sealed class CanChannel : CommunicationChannelBase
{
        ...
        : base(configProvider, logger)
    {
        ...
        ChannelId = $"Can_{mode}_{_localAddress}_{localPort}";
    }
    ...
    public override Task<bool> OpenAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (_mode == ConnectionMode.Server)
            {
                ...
                _logger.Info($"CAN TCP bridge server is listening on {_localAddress}:{_localPort}.");
            }
        ...
        {
            _logger.Error($"Failed to open CAN channel: {ex.Message}", ex);
    ...
    public override async Task<int> SendAsync(string deviceId, ReadOnlyMemory<byte> data, CancellationToken cancellationToken = default)
    {
        ...
        {
            OnErrorOccurred(deviceId, $"CAN frame exceeds maximum bridge frame length {MaxFrameLength}.");
            return 0;
        }
        ...
        {
            OnErrorOccurred(deviceId, $"Failed to send CAN frame: {ex.Message}", ex);
            return 0;
        }
        finally
        {
    ...
    public override async IAsyncEnumerable<ReceivedData> ReceiveAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        throw new NotSupportedException("CanChannel uses event-based data reception (DataReceived event).");
...
#pragma warning restore CS0162
    ...
    private async Task<bool> ConnectClientDeviceAsync(DeviceInfo device, CancellationToken cancellationToken)
    {
        ...
        {
            OnErrorOccurred(device.DeviceId, $"Failed to connect CAN device: {ex.Message}", ex);
            return false;
        }
        finally
        {
    ...
    private async Task<bool> WaitForServerDeviceAsync(DeviceInfo device, CancellationToken cancellationToken)
    {
        ...
        {
            OnErrorOccurred(device.DeviceId, $"Timed out waiting for CAN TCP bridge client on {_localAddress}:{_localPort}.");
            return false;
        }
        finally
        {
    ...
    private async Task AcceptLoopAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested && _listener != null)
        {
            ...
            {
                _logger.Error($"Failed to accept CAN TCP bridge client: {ex.Message}", ex);
            }
            finally
            {
    ...
    private async Task ReceiveLoopAsync(string deviceId, NetworkStream stream, CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested && _isConnected)
        {
            try
            {
                ...
                var frameLength = (prefix[0] << 8) | prefix[1];
                ...
                {
                    connection.AddBytesReceived(PrefixLength + frameLength);
                    connection.LastActiveTime = DateTime.Now;
                }
            ...
            {
                _logger.Debug($"CAN receive loop stopped for device {deviceId}: {ex.Message}");
                break;
            }
            catch (Exception ex)
            {
                OnErrorOccurred(deviceId, $"Failed to receive CAN frame: {ex.Message}", ex);
                break;
            }
        }
